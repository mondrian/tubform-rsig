...
describe "/item_pedido_kits/new.html.erb" do
  include ItemPedidoKitsHelper
  
  before(:each) do
    assigns[:item_pedido_kit] = stub_model(ItemPedidoKit,
      :new_record? => true,
      :pedido_id => 1,
      :quantidade => 9.99,
      :valor_tabela => 9.99,
  ...
  it "renders new item_pedido_kit form" do
    render
    
    response.should have_tag("form[action=?][method=post]", item_pedido_kits_path) do
